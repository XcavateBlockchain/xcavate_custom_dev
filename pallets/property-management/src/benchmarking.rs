...
use scale_info::prelude::{vec, format, vec::Vec};

pub trait Config:
	pallet_marketplace::Config + crate::Config
{
}
...
fn create_registered_property<T: Config>(seller: T::AccountId, region_id: u16, location: LocationId<T>) -> T::AccountId{
    ...
    assert_ok!(Marketplace::<T>::lawyer_claim_property(RawOrigin::Signed(lawyer_1.clone()).into(), 0, LegalProperty::RealEstateDeveloperSide, 400_u32.into()));
    assert_ok!(Marketplace::<T>::lawyer_claim_property(RawOrigin::Signed(lawyer_2.clone()).into(), 0, LegalProperty::SpvSide, 400_u32.into()));
...
fn add_buyers_to_listing<T: Config + pallet_marketplace::Config>(
    ...
    let deposit_amount = property_price
        .saturating_mul(<T as pallet_marketplace::Config>::ListingDeposit::get()) / 100u128.into();
...
mod benchmarks {
    ...
    fn add_letting_agent() {
        ...
        assert_ok!(Whitelist::<T>::add_to_whitelist(RawOrigin::Root.into(), letting_agent.clone()));

        #[extrinsic_call]
        add_letting_agent(RawOrigin::Signed(region_owner), region_id, location, letting_agent.clone());
    ...
    fn letting_agent_deposit() {
        ...
        assert_ok!(PropertyManagement::<T>::add_letting_agent(RawOrigin::Signed(region_owner).into(), region_id, location, letting_agent.clone()));

        #[extrinsic_call]
        letting_agent_deposit(RawOrigin::Signed(letting_agent.clone()));

        assert_eq!(LettingInfo::<T>::get(letting_agent).unwrap().deposited, true);
    }
    ...
    fn add_letting_agent_to_location() {
        ...
        assert_ok!(PropertyManagement::<T>::add_letting_agent(RawOrigin::Signed(region_owner.clone()).into(), region_id, location, letting_agent.clone()));
        ...
        for x in 2..max_locations {
            ...
            assert_ok!(PropertyManagement::<T>::add_letting_agent_to_location(RawOrigin::Signed(region_owner.clone()).into(), new_location, letting_agent.clone()));
        }
        ...
        #[extrinsic_call]
        add_letting_agent_to_location(RawOrigin::Signed(region_owner.clone()), new_location, letting_agent.clone());

        assert_eq!(LettingInfo::<T>::get(letting_agent).unwrap().locations.len(), T::MaxLocations::get() as usize);
    }
    ...
    fn set_letting_agent() {
        ...
        LettingInfo::<T>::insert(
            &letting_agent,
            LettingAgentInfo {
                assigned_properties: BoundedVec::try_from((1..max_props).map(|i| i).collect::<Vec<_>>()).unwrap(),
    ...
    fn distribute_income() {
        ...
        #[extrinsic_call]
        distribute_income(RawOrigin::Signed(letting_agent.clone()), asset_id, distribution_amount, payment_asset);
