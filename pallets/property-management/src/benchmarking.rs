...
fn setup_real_estate_object<T: Config>(
    ...
    let location: BoundedVec<u8, <T as pallet_nft_marketplace::Config>::PostcodeLimit> =
        vec![0; <T as pallet_nft_marketplace::Config>::PostcodeLimit::get() as usize]
            .try_into()
            .unwrap();
    assert_ok!(NftMarketplace::<T>::create_new_location(
        RawOrigin::Root.into(),
        0,
        location.clone()
    ));
...
mod benchmarks {
    ...
    fn add_letting_agent() {
        ...
        assert_eq!(
            PropertyManagement::<T>::letting_info(letting_agent).is_some(),
            true
        );
    }
    ...
    fn letting_agent_deposit() {
        ...
        assert_eq!(
            PropertyManagement::<T>::letting_info(letting_agent)
                .unwrap()
                .deposited,
            true
        );
        assert_eq!(
            PropertyManagement::<T>::letting_agent_locations(0, location).len(),
            1
        );
    }
    ...
    fn add_letting_agent_to_location() {
        ...
        let location1: BoundedVec<u8, <T as pallet_nft_marketplace::Config>::PostcodeLimit> =
            vec![0; <T as pallet_nft_marketplace::Config>::PostcodeLimit::get() as usize]
                .try_into()
                .unwrap();
        let location2: BoundedVec<u8, <T as pallet_nft_marketplace::Config>::PostcodeLimit> =
            vec![1; <T as pallet_nft_marketplace::Config>::PostcodeLimit::get() as usize]
                .try_into()
                .unwrap();
        assert_ok!(NftMarketplace::<T>::create_new_location(
            RawOrigin::Root.into(),
        ...
        assert_eq!(
            PropertyManagement::<T>::letting_agent_locations(0, location1).len(),
            1
        );
        assert_eq!(
            PropertyManagement::<T>::letting_agent_locations(0, location2).len(),
            1
        );
    }
    ...
    fn withdraw_funds() {
        ...
        assert_ok!(PropertyManagement::<T>::distribute_income(
            RawOrigin::Signed(letting_agent).into(),
            0,
            amount
        ));
        let caller: T::AccountId = whitelisted_caller();
        ...
        assert_eq!(
            <T as pallet::Config>::Currency::free_balance(&pallet_account),
            DepositBalanceOf1::<T>::max_value()
        );
        assert_eq!(
            PropertyManagement::<T>::stored_funds(caller.clone()),
            amount
        );
        #[extrinsic_call]
