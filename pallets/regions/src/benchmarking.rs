...
mod benchmarks {
    ...
    fn create_a_new_region<T: Config>(signer: T::AccountId) -> u16 {
        ...
        assert_ok!(Regions::<T>::bid_on_region(RawOrigin::Signed(signer.clone()).into(), region_id, bid_amount));
    ...
    fn propose_new_region() {
        ...
        assert!(!ProposedRegionIds::<T>::contains_key(region.clone().into_u16()));
        ...
        assert!(ProposedRegionIds::<T>::contains_key(region.clone().into_u16()));
        assert_eq!(RegionProposals::<T>::get(region.into_u16()).unwrap().proposal_expiry, expiry_block);
    }
    ...
    fn bid_on_region() {
        ...
        assert_eq!(RegionAuctions::<T>::get(region_id).unwrap().highest_bidder, Some(signer));
    }
    ...
    fn adjust_listing_duration() {
        ...
        #[extrinsic_call]
        adjust_listing_duration(RawOrigin::Signed(signer.clone()), region_id, new_listing_duration);

        assert_eq!(RegionDetails::<T>::get(region_id).unwrap().listing_duration, new_listing_duration);
    }
    ...
    fn create_new_location() {
        ...
        #[extrinsic_call]
        create_new_location(RawOrigin::Signed(signer.clone()), region_id, location.clone());

        assert!(LocationRegistration::<T>::contains_key(region_id, location));
        assert_eq!(RegionDetails::<T>::get(region_id).unwrap().location_count, 1);
    }
    ...
    fn propose_remove_regional_operator() {
        ...
        assert!(OngoingRegionOwnerProposalVotes::<T>::contains_key(region_id));
    }
    ...
    fn vote_on_remove_owner_proposal() {
        ...
        assert_eq!(OngoingRegionOwnerProposalVotes::<T>::get(region_id).unwrap().yes_voting_power, vote_power);
    ...
    fn initiate_region_owner_resignation() {
        ...
        assert_eq!(RegionDetails::<T>::get(region_id).unwrap().next_owner_change, next_owner_change);
    }
    ...
    fn add_regional_operator() {
        ...
        assert!(RegionOperatorAccounts::<T>::contains_key(&regional_operator));
    }
    ...
    fn remove_regional_operator() {
        ...
        assert_ok!(Regions::<T>::add_regional_operator(RawOrigin::Root.into(), regional_operator.clone()));
        assert!(RegionOperatorAccounts::<T>::contains_key(&regional_operator));
