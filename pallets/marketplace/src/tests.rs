...
use crate::{
    OngoingObjectListing, OngoingOffers, PropertyLawyer, RefundToken, TokenListings, TokenOwner,
};
use frame_support::{
...
fn claim_property_token_fails() {
    new_test_ext().execute_with(|| {
        ...
        let expiry =
            frame_system::Pallet::<Test>::block_number() + ClaimWindowTime::get() + 1;
        run_to_block(expiry);
        assert_noop!(
            Marketplace::claim_property_token(RuntimeOrigin::signed([1; 32].into()), 0,),
...
fn finalize_claim_window_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(Marketplace::finalize_claim_window(
            RuntimeOrigin::signed([1; 32].into()),
            0,
...
fn finalize_property_deal_4() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(LocalAssets::balance(0, &[1; 32].into()), 60);
    })
}
...
fn withdraw_legal_process_expired_fails() {
    new_test_ext().execute_with(|| {
        ...
        run_to_block(162);
        assert_noop!(
            Marketplace::withdraw_legal_process_expired(RuntimeOrigin::signed([2; 32].into()), 0,),
            BadOrigin
        );
    })
}
