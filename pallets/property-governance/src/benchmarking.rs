...
fn setup_real_estate_object<T: Config>() -> T::AccountId {
    ...
    <T as pallet_nfts::Config>::Currency::make_free_balance_be(
        &NftMarketplace::<T>::treasury_account_id(),
        max_balance,
    );
    <T as pallet_nfts::Config>::Currency::make_free_balance_be(
        &NftMarketplace::<T>::community_account_id(),
        max_balance,
    );
    ...
    <T as pallet_property_management::Config>::Currency::make_free_balance_be(
        &letting_agent,
        DepositBalanceOf1::<T>::max_value(),
    );
    <T as pallet_property_management::Config>::Currency::make_free_balance_be(
        &PropertyManagement::<T>::account_id(),
        2_u32.into(),
    );
    <T as pallet_property_management::Config>::Currency::make_free_balance_be(
        &PropertyGovernance::<T>::account_id(),
...
mod benchmarks {
    ...
    fn vote_on_proposal() {
        ...
        assert_eq!(
            PropertyGovernance::<T>::ongoing_votes(1).unwrap().yes_votes,
            100
        );
    ...
    fn vote_on_letting_agent_challenge() {
        ...
        assert_eq!(
            PropertyGovernance::<T>::ongoing_challenge_votes(1, crate::ChallengeState::First)
                .unwrap()
                .yes_votes,
            100
        );
        assert_eq!(
            PropertyGovernance::<T>::challenge_voter(1, crate::ChallengeState::First).len(),
            1
        );
    }
