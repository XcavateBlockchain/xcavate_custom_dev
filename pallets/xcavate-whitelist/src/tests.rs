use crate::{mock::*, Error, IsWhitelisted};
...
fn add_to_whitelist_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Whitelist::is_whitelisted(&1), true);
    });
}
...
fn add_to_whitelist_fails_with_no_permission() {
    new_test_ext().execute_with(|| {
        System::set_block_number(1);
        assert_noop!(
            Whitelist::add_to_whitelist(RuntimeOrigin::signed(2), 1),
            BadOrigin
        );
    });
}
...
fn remove_from_whitelist_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Whitelist::is_whitelisted(&1), false);
    });
}
...
fn remove_from_whitelist_fails_with_no_permission() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Whitelist::add_to_whitelist(RuntimeOrigin::root(), 1));
        assert_noop!(
            Whitelist::remove_from_whitelist(RuntimeOrigin::signed(2), 1),
            BadOrigin
        );
    });
}
